...
#if __name__ == '__main__': N_Proces = 16; Pool(N_Proces).map(FUN, zip(np.linspace(0.01,2.0, POINTS), range(POINTS)))
...
def mergeFiles(input):
    for index in range(POINTS):
        ...
        with open(os.path.join(PATH+'/data_2p/ifn_kT1_b_{}.dat'.format(index)), 'w') as f_series:
...
def bistable(input):
    ...
    for index in range(POINTS):
        data = np.genfromtxt(PATH+'/data_2p/ifn_kT1_b_{}.dat'.format(index))
    ...
    np.savetxt(PATH+'/data_2p/ifn_kT1_b_bistable.dat', final_bistable, fmt='%0.5f')
    np.savetxt(PATH+'/data_2p/ifn_kT1_b_transition.dat', final_transition, fmt='%0.5f')    
    
#print (bistable(0))
#data = np.genfromtxt(PATH+'/data_2p/ifn_kT1_b_0.dat'); plt.plot(data[:,1], data[:,2], '.r', data[:,1], data[:,3], '.b', markersize=1.0); plt.show()
